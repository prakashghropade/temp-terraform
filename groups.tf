...
resource "aws_iam_group_membership" "sales_mambers" {
  ...
  users = [
    for user in aws_iam_users : user.name if user.tags.Department == "Sales" 
  ]
...
resource "aws_iam_group_membership" "reception_members" {
    ...
    users = [
        for user in aws_iam_users : user.name if user.tags.Department == "Reception"
    ]
}
...
resource "aws_iam_group_membership" "accounting_members" {
    ...
    users = [
        for user in aws_iam_users : user.name if user.tags.Department == "Accounting"
    ]
}